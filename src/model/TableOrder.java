...
public class TableOrder
{
	...
	/**
	 * Sets the boolean of the table order, to clearly show whether a table of guests have
	 * sent their first table order out in the kitchen. Once a table order has been sent
	 * to the kitchen the status is changed to true and will remain in this state, for
	 * the rest of the table's guests dining session.
	 *
    ...
    /**
     * Sets the closed status of the table order.
     * When the isTableOrderClosed boolean is set to true, it means that the table has finished
     * their dining session and no longer wants to order, and are ready to begin the payment
	 ...
	 * this is being set in the form of an integer to illustrate seconds, the 
	 * reason for this is that seconds level of precision is perceived as a fine
	 * enough level of estimation for this type of task.
	 *
	...
	/**
	 * calculates the estimated time in seconds it would take to prepare the
	 * entire table's order, by summarizing each PersonalOrder instance's 
	 * preparation time, adding the expected delay from before the kitchen receives
	 * the order in their system, and accounts for starting on a new personal order,
	 * by adding 15 seconds per PersonalOrder.
	 *
	 * @return the estimated time it takes for the table order to be prepared.
	 */
/* TODO: Uncomment this line, when working more detailed on preparation time, and reference it in the
 * getOrderPreparationTime method
	public int calculateTableOrderPreparationTime()
	{
		// The maximum of time in seconds we expect it take before the information is relayed and displayed in the kitchen
		int potentialQueueTime = 30;
		
		// The time in seconds that is expected to be added for each individual personal order that will be prepared
		int personalOrderMultiplier = 0;
		
		// The time in seconds of the personal order that takes the longest to prepare
		int longestPersonalOrderPreparationTime = 0;
		
		// The time it is expected to take to prepare the entire table's meal 
		int tableOrderPreparationTime = 0;
		
		// Iterates through the list of personal orders
        for (PersonalOrder personalOrder : listOfPersonalOrders)
        {
        	// If the personalOrder takes longer time to prepare than the current value of longestPersonalOrderPreparationTime then execute this section
        	if(personalOrder.getPreparationTime() > longestPersonalOrderPreparationTime)
        	{
        		// Sets the value of the longestPersonalOrderPreparationTime variable to be the same as the personalOrder's preparation time
        		longestPersonalOrderPreparationTime = personalOrder.getPreparationTime();
        	}
        }
        
        // Adds additional seconds to the preparation time based on each personal order in the list
        personalOrderMultiplier = listOfPersonalOrders.size() * 15;
        
        // Adds the time values in seconds together to find the total time in seconds it will take before the meal is ready
        tableOrderPreparationTime = tableOrderPreparationTime + potentialQueueTime + personalOrderMultiplier;
...
*/
	...
	/**
	 * Sets the payment method that the paying guest has chosen to pay for this table order.
	 * Representing payment choices such as credit card payment or cash payment. 
	 *
	 * @param paymentType a string representing the payment method, 'Credit Card' og 'Cash Payment'
	 */
	...
	/**
	 * Adds a PersonalOrder object to the list of the individual personal orders, that
	 * are associated with this TableOrder instance.
	 *
